...
void paging_init(multiboot_info_t * multiboot) {
	...
	idt_registerHandler(14, pageFault);
...
// The error code gives us details of what happened.
	int present = !(regs->err_code & PAGE_PRESENT); // Page not present
	int rw = regs->err_code & PAGE_WRITE; // Write operation?
	int us = regs->err_code & PAGE_USER; // Processor was in user-mode?
